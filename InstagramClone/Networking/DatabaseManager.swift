...
class DatabaseManager {
    ...
    func searchUser(query: String) async throws -> [AppUser] {
        return try await service.collection(DatabaseConstants.COLLECTION_USERS).whereField("username", isGreaterThanOrEqualTo: query).whereField("username", isLessThanOrEqualTo: query + "\u{f8ff}").getDocuments().documents.compactMap { snapshot in
            try? snapshot.data(as: AppUser.self)
        }
    }

    func uploadStory(story: Story) async throws {
        let encodedStory: [String: Any]
        do {
            encodedStory = try Firestore.Encoder().encode(story)
        } catch {
            throw NSError(domain: "FirestoreEncodingError", code: 4, userInfo: [NSLocalizedDescriptionKey: "Failed to encode story data"])
        }
            
        do {
            try await service.collection(DatabaseConstants.COLLECTION_STORIES).document(story.id).setData(encodedStory)
    ...
    func getAllPosts() async throws -> [Post] {
        return try await service.collection(DatabaseConstants.COLLECTION_POSTS).getDocuments().documents.compactMap { try? $0.data(as: Post.self) }
    }
